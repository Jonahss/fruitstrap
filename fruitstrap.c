...
CFStringRef copy_developer_disk_image_path(AMDeviceRef device) {
    ...
    path = CFStringCreateWithFormat(NULL, NULL, CFSTR("/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/6.1 (10B141)/DeveloperDiskImage.dmg"), home, version, build);
    found = path_exists(path);

    if (!found) {
        path = CFStringCreateWithFormat(NULL, NULL, CFSTR("/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/6.1 (10B141)/DeveloperDiskImage.dmg)"), version, build);
        found = path_exists(path);
    }
    if (!found) {
        path = CFStringCreateWithFormat(NULL, NULL, CFSTR("/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/6.1 (10B141)/DeveloperDiskImage.dmg"), home, version);
        found = path_exists(path);
    }
    if (!found) {
        path = CFStringCreateWithFormat(NULL, NULL, CFSTR("/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/6.1 (10B141)/DeveloperDiskImage.dmg"), version);
        found = path_exists(path);
    }
    if (!found) {
        path = CFStringCreateWithFormat(NULL, NULL, CFSTR("/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/DeviceSupport/6.1 (10B141)/DeveloperDiskImage.dmg"), home);
        found = path_exists(path);
    }
    if (!found) {
...
CFURLRef check_device_app_url(AMDeviceRef device, CFStringRef identifier) {
    CFDictionaryRef result;
    CFURLRef url = NULL;
    assert(AMDeviceLookupApplications(device, 0, &result) == 0);
    
    CFDictionaryRef app_dict = CFDictionaryGetValue(result, identifier);
    if (operation == OP_UNINSTALL && app_dict == NULL) {
        PRINT("There is no app to be uninstall!\n");
...
// Method to get app URL on device
CFURLRef copy_device_app_url(AMDeviceRef device, CFStringRef identifier) {
    ...
    CFStringRef app = CFDictionaryGetValue(app_dict, CFSTR("Path"));
    PRINT("Printing app: %s\n", CFStringGetCStringPtr(app, CFStringGetSystemEncoding()));
    
    assert(app != NULL);
    
    CFURLRef url = CFURLCreateWithFileSystemPath(NULL, app, kCFURLPOSIXPathStyle, true);
    CFRelease(result);
    return url;
}

// Method to retrive bundle version from the location - unfortunately works just for local path (not device)
CFStringRef get_bundle_version(CFURLRef disk_app_url) {
    PRINT("url");
    ...
    CFStringRef bundle_version = CFRetain(CFDictionaryGetValue(plist, CFSTR("CFBundleVersion")));
...
void handle_device(AMDeviceRef device) {
    ...
    CFURLRef urlApp = checkAppExists(device);
...
void usage(const char* app) {
    ...
    printf ("   install    [--id device_id] --bundle appPath.app [-a/--args arguments] \n");
    ...
    printf ("   uninstall  [--id device_id] --bundle bundle.app \n");
    ...
    printf ("   isInstalled  [--id device_id] --bundle bundle.app \n");
    printf ("    * Check the specified bundle identifier (eg com.foo.MyApp) from the specified device, or all attached devices if none are specified. \n\n");
    printf ("   list-devices  \n");
...
int main(int argc, char *argv[]) {
    ...
    } else if (strcmp (argv [optind], "isInstalled") == 0) {
        operation = OP_ISINSTALLED;
    } else {
        usage (argv [0]);
        exit (0);
    }
